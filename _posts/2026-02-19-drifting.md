...
We want $q_\theta \approx p_r$ where $p_r$ is the distribution of true images and $q_\theta$ is the distribution after pushforward model $q_{\theta} = f_{\theta}(p_{prior})$ and $p_{prior} \sim \mathcal{N}(0,I)$. The natural objective is:
...
Since $\nabla_\mathbf{x}\, k(\mathbf{x}, y^-_i) = -\dfrac{\mathbf{x}-y^-_i}{\tau^2} k(\mathbf{x}, y^-_i)$, letting $\tilde{k}(\mathbf{x}, y^-_i) = \dfrac{k(\mathbf{x}, y^-_i)}{\sum_i k(\mathbf{x}, y^-_i)}$ be the normalized weights:
$$
...
This is the mean-shift vector — a weighted average pulling $\mathbf{x}$ toward nearby generated samples $\{y^-_i\} \sim q_\theta$. Identically for $p_r$ with real samples $\{y^+_j\} \sim p_r$:

$$
\nabla_\mathbf{x} \log p_r(\mathbf{x}) = \frac{1}{\tau^2}\sum_j \tilde{k}(\mathbf{x}, y^+_j)(y^+_j - \mathbf{x}) =: \frac{1}{\tau^2} V^+_p(\mathbf{x})
$$
## 4. The Drifting Field
...
If the model can accurately push the prior distribution to image distribution, the equilibrium $V_{p,q}(\mathbf{x}) = 0$, so $\mathbf{x} + V_{p,q}(\mathbf{x}) = \mathbf{x}$. This motivates chasing a frozen drifted target at each training step:
...
$$
D_\mathrm{KL}(q_\theta \| p_r) \xrightarrow{\ \nabla_{\mathbf{x}_i}\ } \underbrace{\mathbf{s}_r(\mathbf{x}_i) - \mathbf{s}_q(\mathbf{x}_i)}_{\text{score diff.}} \xrightarrow{\ \text{KDE}\ } \underbrace{V^+_p(\mathbf{x}_i) - V^-_q(\mathbf{x}_i)}_{\text{mean-shift}} \xrightarrow{\ \text{fixed pt.}\ } \mathcal{L} = \mathbb{E}\!\left[\|V_{p,q}\|^2\right]
$$